...
__all__ = ["AlertRepositoryImpl", "NotFoundError", "RepositoryError"]
...
class AlertRepositoryImpl(AlertRepository):
    ...
    def _model_to_entity(self, model: AlertModel) -> Alert:
        ...
        return Alert(
            ...
            assigned_analyst_id=UUID(str(model.assigned_analyst_id))
            if model.assigned_analyst_id is not None
            else None,
            status=model.status,
